                            ...
                            {cartItems.map((item) => (
                                <ListGroup.Item key={item._id}>
                                    <Row className="align-items-center">
                                        ...
                                        <Col md={3}>
                                            <Button variant="light" disabled={item.quantity == 1} onClick={() => handleUpdateCart(item, item.quantity-1)}>
                                                <i className="fas fa-minus-circle"></i>
                                            </Button>{' '}
                                            <span>{item.quantity}</span>{' '}
                                            <Button variant="light" onClick={() => handleUpdateCart(item, item.quantity+1)} disabled={item.quantity == item.countInStock}>
                    ...
                    <Card>
                        <Card.Body>
                            <ListGroup variant="flush">
                                    <ListGroup.Item>
                                        <h3>
                                            Subtotal ({cartItems.reduce((sum, item) => sum + item.quantity, 0)} items) : ${cartItems.reduce((sum, item) => sum + item.price * item.quantity, 0)}
                                        </h3>
                                    </ListGroup.Item>
                                    <ListGroup.Item>
                                        <div className="d-grid">
                                            <Button type="button" variant="primary" disabled={cartItems.length == 0} onClick={() => handleCheckout()}>
                                                Proceed to Checkout
                                            </Button>
                                        </div>
                                    </ListGroup.Item>
